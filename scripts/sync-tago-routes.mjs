...
console.log(`TAGO 버스노선 동기화를 시작합니다. 도시 ${cityCodes.length}개 · ${dryRun ? "미리보기" : "Supabase 저장"}`);

for (const cityCode of cityCodes) {
  const fetched = await fetchTagoRoutesForCity({ cityCode, serviceKey });
  const normalized = fetched.items
    .map((item) => normalizeTagoRoute(item, cityCode, fetchedAt))
    .filter(Boolean);
  const unique = [...new Map(normalized.map((route) => [`${route.city_code}:${route.local_id}`, route])).values()];
  ...
  if (admin) {
    ...
    const { count, error } = await admin
      ...
      .eq("source", "tago_bus_route")
      .eq("city_code", cityCode);
    if (error) throw new Error(`${cityCode}: Supabase 저장 결과 확인 실패 - ${error.message}`);
    databaseRows = count;
  }
  ...
  const databaseLabel = databaseRows === null ? "" : `, DB 확인 ${databaseRows.toLocaleString()}개`;
  console.log(`${status} ${cityCode}: ${unique.length.toLocaleString()}개 ${dryRun ? "정규화" : "저장"} (${fetched.requestCount}회 호출, 제외 ${skipped}개${databaseLabel})`);
}
...
const markdown = [
  ...
  `- 도시: ${cityCodes.join(", ")}`,
  `- 정규화된 노선: ${totalSaved.toLocaleString()}개`,
  `- TAGO 호출: ${totalRequests.toLocaleString()}회`,
  "",
  ...summaries.map((result) => {
    const databaseLabel = result.databaseRows === null ? "" : `, DB 확인 ${result.databaseRows.toLocaleString()}개`;
    return `- ${result.cityCode}: ${result.saved.toLocaleString()}개 (호출 ${result.requests}회, 제외 ${result.skipped}개${databaseLabel})`;
  }),
  "",
].join("\n");

if (process.env.GITHUB_STEP_SUMMARY) await appendFile(process.env.GITHUB_STEP_SUMMARY, markdown, "utf8");
assertEveryCityHasRoutes(summaries);
